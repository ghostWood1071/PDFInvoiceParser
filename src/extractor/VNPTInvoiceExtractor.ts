...
export class VNPInvoiceExtractor extends PdfExtractor {
  ...
  protected processPage(pageLines: string[]) {
    ...
    result.serial = this.getBehind(lineTmp.strResult, ":").replace(
      new RegExp("#", "g"),
      ""
    );
    lineTmp = this.getUntil(pageLines, lineTmp.nextPos, "NGƯỜI MUA HÀNG");
    result.no = this.getBehind(lineTmp.strResult, ":").replace(
      new RegExp("#", "g"),
      ""
    );

    nextPos =
      this.getUntil(pageLines, lineTmp.nextPos, "i website: ").nextPos + 1;
    ...
    for (let linePos = nextPos; linePos < pageLines.length; linePos++) {
      if (exchangeRegex.test(pageLines[linePos])) {
        ...
        if (tmp_rate != ":") result.exchange_rate = parseFloat(tmp_rate);
        break;
      }
      if (rateVATstartRegex.test(pageLines[linePos])) {
        result.vat_rate = parseFloat(
          pageLines[linePos].split("#")[3].replace("%", "")
        );
      }
