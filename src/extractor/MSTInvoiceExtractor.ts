...
export class MSTInvoiceExtractor extends PdfExtractor {
  ...
  private processPage(pageLines: string[]) {
    ...
    if (pageLines.length == 0) return pageResult;
    ...
    //get seller
    let sellerNameResult = this.getUntil(
      pageLines,
      tmpL.nextPos,
      "Mã số thuế:"
    );
    pageResult.seller.companyName = this.getBehind(
      sellerNameResult.strResult,
      ":"
    );
    let sellerTaxResult = this.getUntil(
      pageLines,
      sellerNameResult.nextPos,
      "Địa chỉ:"
    );
    ...
    nextPos =
      this.getUntil(
        pageLines,
        sellerTaxResult.nextPos,
        "Họ tên người mua hàng:"
      ).nextPos + 1;
    let buyerNameResult = this.getUntil(pageLines, nextPos, "Địa chỉ:");
    pageResult.buyer.companyName = this.getBehind(
      buyerNameResult.strResult,
      ":"
    );
    nextPos =
      this.getUntil(pageLines, buyerNameResult.nextPos, "Số tài khoản:")
        .nextPos + 1;
    ...
    for (let linePos = nextPos; linePos < pageLines.length; linePos++) {
      ...
      if (rateVATstartRegex.test(pageLines[linePos])) {
        pageResult.vat_rate = parseFloat(
          this.execRegex(pageLines[linePos], rateVATRegex)
            .replace(":", "")
            .replace("%", "")
        );
        break;
      }
      if (endTableRegex.test(pageLines[linePos])) break;
