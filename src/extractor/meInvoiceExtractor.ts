...
export class meInvoiceExtractor extends PdfExtractor {
  ...
  private processTableRow(rowStr: string) {
    ...
    if (rowStr.endsWith("#")) rowStr = rowStr.substring(0, rowStr.length - 2);
  ...
  private processPage(pageLines: string[]) {
    ...
    let enTableRegex =
      /Cộng tiền|Hình thức thanh toán|Người mua hàng|Số:#|\d+\/\d+|\(Theo PO đặt hàng số/;
    ...
    for (let linePos = nextPos; linePos < pageLines.length; linePos++) {
      if (!pageLines[linePos].includes("STT#Tên hàng hóa")) {
        ...
        line =
          line + pageLines[linePos] + (pageLines[linePos] == "" ? "" : "#");
      }
    }

    if (rowRegex.test(line)) result.table.push(this.processTableRow(line));
    return result;
  }
